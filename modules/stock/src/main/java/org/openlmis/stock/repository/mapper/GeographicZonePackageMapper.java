...
public interface GeographicZonePackageMapper extends HasGeographicZone,HasVaccinePackaging,HasUser,StockMapper<GeographicZonePackage>{
    ...
    @Results(value = {
            ...
            @Result(property = "destination_geographic_zone", javaType = GeographicZone.class, column = "destination_geographic_zone_id",
                    one = @One(select = "getGeographicZoneById")),
    ...
    @Insert("insert into geographic_zone_package (package_number, number_of_packages, date_sent, date_recieved, recieved_status, sending_user_id, receiving_user_id, facility_id, source_geographic_zone_id, destination_geographic_zone_id, vaccine_packaging_id) values " +
    ...
    List<GeographicZonePackage> filter(@Param("filter") String filter,GeographicZonePackage geographicZonePackage);
